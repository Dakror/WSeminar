...
Im weiteren Verlauf wird es als \textit{PathFinder} bezeichnet.\\
Geschrieben ist die Anwendung in der Programmiersprache \textit{Java} unter Verwendung der \textit{JavaFX}-Standardbibliothek \cite{javafx}.
\begin{figure}[h!]
...
\textit{Seed}-Eingabefeld ausgestattet.  %\footnote{\hyperref[sec:basics]{Abschnitt \ref*{sec:basics} - \nameref{sec:basics}}}
\\
...
\textit{Kante} im \textit{Graphen} platziert (\autoref{alg:generatorv2})\footnote{vgl. Anhang: GraphGenerator.java}.
...
\\
Der in \textit{PathFinder} verwendete Algorithmus ist der \textit{Fruchtermann-Reingold} Algorithmus, welcher 1991 von Thomas M. J. Fruchterman und Edward M. Reingold an der University of Illinois veröffentlicht wurde.\cite{fruchterman}
\newpage
